        ...
        if request.method == "GET":
            if request.GET.get("category"):
                posts = Movie.objects.filter(
                    category__name__in=request.GET.getlist("category")
                )
                return render(
                    request,
                    "authentication/all_films.html",
...
class FilterMovieView(ListView):
    ...
    extra_context = {
        "categories": Category.objects.all(),
        "years": list(
            Movie.objects.all().values_list("year", flat=True).distinct("year__year")
        ),
    }

    def get_queryset(self):
        posts = Movie.objects.filter(
            category__name__in=self.request.GET.getlist("category")
            or Movie.objects.all().values_list("category__name"),
            year__year__in=self.request.GET.getlist("year")
            or Movie.objects.all().values_list("year__year"),
        ).distinct("name")
        return posts
